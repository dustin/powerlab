...
func statusLogger() {
	...
	for range time.Tick(*stateLogFreq) {
		...
		case powerlab.Charging, powerlab.TrickleCharging:
			log.Printf("%v%s %vS %.1f%%, current=%.2fA, in=%vmA, cells=%v, ir=%v, charge time=%v",
				st.Mode(), st.DetectedCellCount(), complete, st.AvgCell(), st.AvgAmps(),
				st.MAhIn(), volts, ir, st.ChargeDuration())
		case powerlab.Discharging:
			log.Printf("%v%s %vS %.1f%%, current=%.2fA, out=%vmA, cells=%v, discharge time=%v",
				st.Mode(), st.DetectedCellCount(), complete, st.AvgCell(), st.AvgAmps(),
